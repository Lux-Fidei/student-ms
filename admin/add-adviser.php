...
if (strlen($_SESSION['sturecmsaid']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $uid = createAccount($uname, $password, 'Adviser');
        ...
        if (in_array($extension, $allowed_extensions)) {
            ...
            echo '<script>alert("Profile picture has invalid format. Only jpg / jpeg/ png /gif formats allowed.")</script>';
        }
    }

    // Fetch class combinations for the advisory class dropdown
    $sql = "SELECT DISTINCT CONCAT(Strand, ' - ', GradeLevel, ' - ', section) AS ClassCombination FROM tblstudent";
    $query = $dbh->prepare($sql);
    $query->execute();
    $class_combinations = $query->fetchAll(PDO::FETCH_OBJ);
?>
            ...
            <div class="content-wrapper">
                ...
                <div class="row">
                    <div class="col-12 grid-margin stretch-card">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title" style="text-align: center;">Adviser</h4>
                                            ...
                                            if (position === 'Chairperson (Subject)') {
                                                ...
                                                var classOptions = `
                                                    <div class="form-group">
                                                        ...
                                                        <select name="advisory_class" class="form-control" required>
                                                            <option value="">Select Advisory Class</option>
                                                            <?php foreach ($class_combinations as $class) { ?>
                                                                <option value="<?php echo htmlentities($class->ClassCombination); ?>"><?php echo htmlentities($class->ClassCombination); ?></option>
                                                            <?php } ?>
                                                        </select>
                                                    </div>
                                                `;
                                                additionalFields.innerHTML = classOptions;
                                            } else {
                                                additionalFields.innerHTML = '';
                                            }
                                        });
